...
cp runCond.sub $outcond             # 1. cond submit script
...
do
  ...
  var3=`expr 2 `                    # number of events (dnbb)
  var4=`expr 3 `                    # partitions (dnbb)
  var5=`expr 4 `                    # Current partition number (dnbb)
  var6=`expr 5 `                    # path to the ntuples (this if of interest)
  col1=${array[$var1]}  
  col2=${array[$var2]}
  if [ $col2 -ne $lineNum ]
  then
  ...
  IFS='/' read -r -a array2 <<< "$col6"  # trim the path to the ntuples
  len2=${#array2[@]}                # must be --
  ...
  echo $count  $iFile               # iFile is the trimmed ntuple name
  ...
  cp runCond.sub $iFile             # copy cond submit script to iFile
  cp runAnalysis.sh $iFile          # pass the argument script (for LineNumber, doQC, syst and dir) 
  cd $iFile                         # cd to iFile
  sed -i "s:LINENUM:$lineNum:g" runCond.sub   # dictate which file to process
  if [ -z "$doQCD" ]
  then
    for qcdnum in {0..3}  # full doQCD
    do
      sed -i "s:DOQCD:$qcdnum:g" runCond.sub
      if [ -z "$systematics" ] && [ -z "$direction" ]
      then
        sed -i "s:SYSTEMATICS:$allSystematics:g" runCond.sub
        sed -i "s:DIRECTION:$allDirection:g"     runCond.sub
        condor_submit runCond.sub ###this is already properly indented
      elif [ -z "$direction" ] 
      then
        sed -i "s:SYSTEMATICS:$systematics:g"    runCond.sub
        sed -i "s:DIRECTION:$allDirection:g"     runCond.sub
        condor_submit runCond.sub ###this is already properly indented
      else
        sed -i "s:SYSTEMATICS:$systematics:g"    runCond.sub
        sed -i "s:DIRECTION:$direction:g"        runCond.sub
        condor_submit runCond.sub ###this is already properly indented
      fi
    done
  else 
    sed -i "s:DOQCD:$doQCD:g" runCond.sub   # passed doQCD 
    if [ -z "$systematics" ] && [ -z "$direction" ]
    then
      sed -i "s:SYSTEMATICS:$allSystematics:g" runCond.sub
      sed -i "s:DIRECTION:$allDirection:g"     runCond.sub
