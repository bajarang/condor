  ...
  */
  ...
  if(trimmedFileName.find("_Data_")!=string::npos) {
    for(int i=0; i<NSystData; i++){
      cout << dataSyst[i] << "  " << dataDir[i] << endl;
      if      (i==0)   { 
        dataSystDirAppendString[i] << "_Syst" << dataSyst[i] << "_CN";
      }
      else if (i==1)   { 
        dataSystDirAppendString[i] << "_Syst" << dataSyst[i] << "_UP";
      }
      else             { 
        dataSystDirAppendString[i] << "_Syst" << dataSyst[i] << "_DN";
      }
    }
  }
  midAppendString << "_JetPtMin_30_VarWidth_BVeto_QCD";
  ...
  for(int i=0; i<3; i++){
    for(int j=0; j<4; j++){
      cout << trimmedFileName << beginAppendString.str() << dataSystDirAppendString[i].str() << midAppendString.str() << j << endAppendString.str() << endl;
    }
  }
  */
  ...
  //////////////////////////////////////////////////////////////////
    ...
    if(intSystematics!=2 && intSystematics!=4) {
      beginAppendString << "_EffiCorr_1";
      beginAppendString << "_TrigCorr_1";
    }
    else {
      cout << "We do not carry out systematics = " << intSystematics  << " for Signal MC " << endl;
      cout << "Exiting Now." << endl;
      exit(0); 
    }
  }
  ...
  //////////////////////////////////////////////////////////////////
  beginAppendString << "_Syst_" << systematics;
  if(direction == "0")        beginAppendString << "_CN";
  else if (direction == "1")  beginAppendString << "_UP";
  else if (direction == "-1") beginAppendString << "_DN";
  beginAppendString << "_JetPtMin_30"; 
  beginAppendString << "_BVeto";
  beginAppendString << "_QCD" << doQCD;
  beginAppendString << "_MET15";
  beginAppendString << "_mT50";
  beginAppendString << ".root"; 

  trimmedFileName = trimmedFileName + beginAppendString.str();
  cout << "--> Output histogram filename : " << trimmedFileName << endl << endl;
  */
  /*
  ...
  int possibleSystematics[10] = {0, 1, 2, 3, 4, 5, 6, 7, 8, 12};
  ...
  if(!matchesSysteamtics) {
    ...
    cout << "Please choose one among these values : 0  1  2  3  4  5  6  7  8  12" << endl;
    cout << "Exiting Now." << endl << endl;
    exit(0);
  }
